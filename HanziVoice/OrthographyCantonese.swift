...
class OrthographyCantonese: OrthographyImpl {
    ...
    func getAllTones(_ string:String) -> [String] {
        ...
        let isEnteringTone = "ptk".characters.contains(stem.last!)
        
        if (tone != "1")                    { result.append(stem + "1") }
        if (tone != "2" && isEnteringTone)  { result.append(stem + "2") }
        if (tone != "3")                    { result.append(stem + "3") }
        if (tone != "4" && isEnteringTone)  { result.append(stem + "4") }
        if (tone != "5" && isEnteringTone)  { result.append(stem + "5") }
